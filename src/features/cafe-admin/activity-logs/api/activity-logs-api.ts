...
/**
 * Получить логи активности для кафе с фильтрацией
 */
export async function getActivityLogs(
  ...
  const response = await fetch(`/api/cafe-admin/activity-logs?${params.toString()}`, {
    cache: 'no-store',
    credentials: 'include',
  });
...
/**
 * Получить статистику по логам для кафе
 */
export async function getActivityLogsStatistics(
  ...
  const response = await fetch(`/api/cafe-admin/activity-logs/statistics?${params.toString()}`, {
    cache: 'no-store',
    credentials: 'include',
  });
