...
export function EditTaskModal({ open, onClose, template, onSuccess }: EditTaskModalProps) {
  const [formData, setFormData] = useState<UpdateTaskTemplateDto>({});
  ...
  useEffect(() => {
    if (template) {
      setFormData({
        ...
        assignedWorkerIds: template.assignedWorkerIds,
  ...
  const currentAssignmentType =
    (formData.assignmentType as TaskAssignmentType | undefined) ??
    (template?.assignmentType as TaskAssignmentType | undefined);

  useEffect(() => {
    if (!open) return;
    if (!template) return;
    if (currentAssignmentType !== TaskAssignmentType.SPECIFIC_WORKERS) return;
  ...
  }, [open, template, currentAssignmentType]);
                ...
                return {
                  ...
                  assignedWorkerIds:
                    nextType === TaskAssignmentType.SPECIFIC_WORKERS
                      ? (s.assignedWorkerIds ?? template.assignedWorkerIds ?? [])
                      : [],
                  assignedRoles:
                    nextType === TaskAssignmentType.ROLE_BASED
                      ? (s.assignedRoles ?? template.assignedRoles ?? [])
                      : [],
                };
              })
            }
